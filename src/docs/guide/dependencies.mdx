...
`systemic` is a modifier for dependencies to indicate that the dependency isn't directly modifiable (`sealed`) and is from the system.

So when importing, kaoscript won't try to overwrite that dependency.

### implements & native class

`impl` doesn't directly modify classes that have been flagged as `sealed` (a `systemic` class is `sealed`).
