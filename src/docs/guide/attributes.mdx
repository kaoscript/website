...
|      assert-parameter      |                     the parameters aren't asserted (existence, type)                     |
|   dont-assert-parameter    |                      the parameters are asserted (existence, type)                       |
|   assert-parameter-type    |                          the parameters' types aren't asserted                           |
| dont-assert-parameter-type |                            the parameters' types are asserted                            |
|     assert-new-struct      |                         the values of a new struct are asserted                          |
|   dont-assert-new-struct   |                        the values of a new struct aren't asserted                        |
|      assert-new-tuple      |                          the values of a new tuple are asserted                          |
|   dont-assert-new-tuple    |                        the values of a new tuple aren't asserted                         |
...
`assert-parameter`/`dont-assert-parameter` enables/disables all the assertions of the parameter, its existence, its type, ... -->

### assert-parameter-type

<!-- `assert-parameter-type`/`dont-assert-parameter-type` enables/disables the type checking of the arguments. -->

If the type checking is disable, a function might generate unexpected runtime errors.

<!-- ### assert-new-struct

`assert-new-struct`/`dont-assert-new-struct` enables/disables all the assertions of the values given to create a new struct.

### assert-new-tuple

`assert-new-tuple`/`dont-assert-new-tuple` enables/disables all the assertions of the values given to create a new tuple.
