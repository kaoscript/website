...
// anything but not primitives or arrays
```
...
x:!(String | Number).toFloat()
```

## type assertion

At runtime, `:&()` throws an error when the value doesn't match the type.

You can use `:&?()` to return `null` when the value doesn't match the type.
...
At runtime, `:>()` throws an error when the value can't be casted.

You can use `:>?()` to return `null` when the value can't be casted.
...
limited-type = (*func-type* | *object-type* | *array-type* | *named-type*)

generic-type = *entity-type* (',' *entity-type*)\*
entity-type = *varname* ('.' *varname*)\*

named-type = *varname* ('.' *varname*)\* ['<' *limited-type* ([,] *limited-type*)\* '>'] [ *subtype* ]

subtype = '(' ( *varname* ([,] *varname*)\* | *expression* ) ')'
