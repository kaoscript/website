...
```kaoscript
import 'npm:@zokugun/lang'
import 'npm:@zokugun/lang.color'(...)
```
...
Node modules need to use the `node:` scheme  (`import 'node:fs'`).

### `npm:` scheme

Importing npm modules is done by using the `npm:` scheme (`import 'npm:@zokugun/lang'`)
...
import-declarator = "*path*" ['(' *parameter1*, ..., *parameterN* ')' | '(...)'] [(
	...
	|
	=> (*varname* [, *destructuring*] | *descriptive-type*)
)]
