...
if
	*expression:Boolean*
	...
	|
	[*expression:Boolean*] ;; var [mut] (*varname* | *destructuring*) [: *type*] (?= | ?#= | ?|= | ?+=) *expression*
{
	*...statements*
}
(else if
...
```kaoscript
for var x in 0..10..2 while test(x) {
}
```
	...
	var [mut] *varname* [: *type*] = *expression*
{
	(
		(
			*match-condition*
			[with (var [mut] (*varname* [: *type*] | *destructuring*) | *varname* | *destructuring*)]
			[when *expression:Boolean*]
		)
		|
		(
...
match age() {
	...
	else		with var n	=> console.log(`I'm an old person of age \(n)`)
}
```
...
match somePoint {
	with var [x, y] when x == 0 && y == 0				=> console.log(`(0, 0) is at the origin`)
	with var [x, y] when y == 0							=> console.log(`(\(x), 0) is on the x-axis`)
	with var [x, y] when x == 0							=> console.log(`(0, \(y)) is on the y-axis`)
	with var [x, y] when -2 <= x <= 2 && -2 <= y <= 2	=> console.log(`(\(x), \(y)) is inside the box`)
	with var [x, y]										=> console.log(`(\(x), \(y)) is outside of the box`)
...
match value {
	{foo: 1}	with var {qux: n} 		=> console.log(`qux: \(n)`)
