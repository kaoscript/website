...
The type validation can happen at different times depending on the type of the passed argument.

|                               | `foo(x)`  | `foo(x: String)` |
| :---------------------------- | :-------: | :--------------: |
| `foo(null)`                   | `compile` |    `compile`     |
| `foo(x)` where `x` is untyped | `runtime` |    `runtime`     |
...
```kaoscript
var id = parseInt(enteredId)
var id = parseInt <| enteredId
var id = enteredId |> parseInt



var id = #enteredId ? parseInt(enteredId) : null
var id = parseInt #<| enteredId
var id = enteredId |># parseInt
...
Unlike JavaScript, there is no shorthand for functions in object.
